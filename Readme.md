...
This website is targeted at people who want to have their recipes stored online and share them with the other users in order to see what they think of them. The website can also be used by people who are only interested in finding recipes to get inspired.
...
- As a user I want to be able to vote for recipes added by other users.
...
- As a user who likes to keep the app tidy I want to be able to remove types of meal/cuisines that don't have any recipes associated with them.

- As a user who likes to get an overall idea of what type of recipes are saved in the app I would like to see some statistics about the recipes (i.e. recipe charts by difficulty, category, cuisine...)
...
- As a user who has experienced any issues with the app or has any questions I want to be able to contact Time2Eat .
...
- **Social media links:** They appear on the page footer on every website page and by clicking on them the user will get re-directed to the media websites to follow Time2Eat. (since the website is fictitious they will be redirected to the login page for Facebook or Twitter).
...
- Users already registered can click on the link "Login here" or on the logo to get redirected to the login page.
...
- **Search input box**: On the top left corner of the page the users can enter text to search for a specific word in the recipes database. As a result all the recipes containing that word will be displayed. This can be very useful if users are looking for specific ingredients, name of authors, level of difficulty, cuisine, name of recipe or allergens.

- **Carousel:** The first time the user lands on this page will be able to see all the recipes available in the app by moving sideways using the carousel. The recipes in the carousel will be sorted by the number of votes and then by the number of views, with the recipe with most votes at the top of the carousel. If the user moves the carousel to the right the recipes will be sorted descending by number of votes and number of views and if the user moves to the left then the recipes will be sorted in ascending order.
...
- **Filter form**: Below the carousel there is a filter form. This form is taken from [Materialize](<[https://materializecss.com/](https://materializecss.com/)>) and all the input fields inside the form are also coming from [Materialize](<[https://materializecss.com/](https://materializecss.com/)>). The user will be able to filter the results based on the following criteria:
...
- **Type of Meal**: The users can tick the check boxes for the type of meal(s) that they are looking for recipes on.

### View recipe page

This page will display the main attributes for the recipe. If the recipe belongs to the user he/she will have the option to remove or edit the recipe and if the recipes doesn't belong to the user then the user will have the option to vote for the recipe.
...
- **Vote button:** This button will only be visible if the recipe does not belong to the user and by clicking on it the number of votes displayed at the right top of the screen will be increased by one. A user can only vote for the same recipe once and if he/she tries to vote more than once an error message will be displayed.
...
- **Input (type text)** I have used input type text for name of the recipe and calories inputs. Name of recipes is required but calories is optional as not everyone may know the relevant information about a recipe. If the calories are not entered then the text 'Not specified' will be used in the Calories section.
...
- **Textarea:** I have used textareas for the fields "Instructions" and "Ingredients".
  ...
  The user needs to enter every ingredient/instruction on a different line to make sure it displays correctly in the view recipe page and also to be able to convert the input to array.
...
- **Flash message** If the type of meal or cuisines entered already exist in the app then an error message will be displayed to let the user know.
...
This page will give the user an idea of how many and what kind of recipes can be found on the app based on different criteria and also displays a ranking of the top 10 most rated authors.
...
- **Favourites:** Adding an additional filter to display only display the favourites recipes. I thought of this feature once the website was finished and I couldn't spend more time working on the project as I needed to get going with the rest of the course.
...
- **[JavaScript:](https://developer.mozilla.org/en-US/docs/Web/JavaScript)** This is the language used to add interactivity to the website. It has been used to create the charts in the statistics page and the function to preview the image uploaded using input type file.
...
My mentor Guido helped me with setting up unittest for the project and together we created the first two tests (test_string_to_array and test_insert_recipe).

The rest of the tests were created by myself.
...
In order to fix this issue I updated the timeout function in the materialize.js code replacing the 0 with 500. In post below I read that I should update the timeout function to 100 instead of 0 but when I tried that I was still experiencing the issue so I decided to use a longer timeout (500) which fixed the issue.
...
3. I have created a .gitignore file and I have added the files and folder that don't need to be committed (i.e. '.venv' folder, env.py, etc)
...
11. Once the website had been fully tested and was working correctly the changes applied to the staging app were promoted to the Production app:
...
I used the following bootstrap login design to help me create the login/register pages, which I then customised by changing the colours, image and the bootstrap grid.
...
I learnt how to set up the connection between mongodb and DC/JS Crossfilter by watching the following tutorial in the link below:
...
I could fix the bug with the materialize select dropdown in the latest versions of Chrome by reading the following post
